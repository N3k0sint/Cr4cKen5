...
class CyberDashboard:
    def __init__(self, root):
        self.root = root
        self.root.title("Cr4cKen5 - BruteForce Security Suite (Windows Version)")
        ...
        # Config Storage Configuration (Windows Profile)
        self.config_file = os.path.expanduser("~/.cracken5_config_win.json")
        self.config_data = self.load_config()
        
        # Ensure default fallback wordlist exists in Temp folder
        self.default_wl_path = os.path.join(tempfile.gettempdir(), "cracken5_default_wordlist.txt")
        self.extract_default_wordlist()
    ...
    def setup_target_card(self):
        ...
        self.target_entry = tk.Entry(target_card, textvariable=self.target_var, bg=self.bg_color, fg=self.header_color, insertbackground=self.header_color, font=("Consolas", 10), bd=0, highlightbackground=self.button_color, highlightthickness=1)
    ...
    def setup_nmap_frame(self):
        ...
        ttk.Checkbutton(opts_frame, text="Perform OS fingerprinting (-O - Requires Administrator)", variable=self.nmap_o, command=self.update_previews).pack(anchor="w", pady=5)
    ...
    def setup_dirb_frame(self):
        ...
        self.wordlist_var = tk.StringVar(value=self.config_data.get("dirb_wl", self.default_wl_path))
    ...
    def setup_wfuzz_frame(self):
        ...
        entry_frame = tk.Frame(path_frame, bg=self.card_color)
        entry_frame.pack(fill="x")
        self.wfuzz_wl_var = tk.StringVar(value=self.config_data.get("wfuzz_wl", self.default_wl_path))
        ...
        self.wfuzz_hc_entry = tk.Entry(filter_frame, textvariable=self.wfuzz_hc, bg=self.bg_color, fg=self.header_color, insertbackground=self.header_color, font=("Consolas", 10), bd=0, highlightbackground=self.button_color, highlightthickness=1)
        self.wfuzz_hc_entry.grid(row=0, column=1, sticky="ew", padx=(10, 20), pady=5, ipady=3)
        self.wfuzz_hc_entry.bind("<KeyRelease>", self.update_previews)
        
        tk.Label(filter_frame, text="Hide Characters (--hch):", bg=self.card_color, fg=self.text_color, font=("Segoe UI", 10)).grid(row=0, column=2, sticky="w", pady=5)
        self.wfuzz_hh = tk.StringVar(value=self.config_data.get("wfuzz_hh", ""))
        self.wfuzz_hh_entry = tk.Entry(filter_frame, textvariable=self.wfuzz_hh, bg=self.bg_color, fg=self.header_color, insertbackground=self.header_color, font=("Consolas", 10), bd=0, highlightbackground=self.button_color, highlightthickness=1)
        self.wfuzz_hh_entry.grid(row=0, column=3, sticky="ew", padx=10, pady=5, ipady=3)
        self.wfuzz_hh_entry.bind("<KeyRelease>", self.update_previews)
        
        # Row 1: Hide Words & Hide Lines
        tk.Label(filter_frame, text="Hide Words (--hw):", bg=self.card_color, fg=self.text_color, font=("Segoe UI", 10)).grid(row=1, column=0, sticky="w", pady=5)
        self.wfuzz_hw = tk.StringVar(value=self.config_data.get("wfuzz_hw", ""))
        self.wfuzz_hw_entry = tk.Entry(filter_frame, textvariable=self.wfuzz_hw, bg=self.bg_color, fg=self.header_color, insertbackground=self.header_color, font=("Consolas", 10), bd=0, highlightbackground=self.button_color, highlightthickness=1)
        self.wfuzz_hw_entry.grid(row=1, column=1, sticky="ew", padx=(10, 20), pady=5, ipady=3)
        self.wfuzz_hw_entry.bind("<KeyRelease>", self.update_previews)
        
        tk.Label(filter_frame, text="Hide Lines (--hl):", bg=self.card_color, fg=self.text_color, font=("Segoe UI", 10)).grid(row=1, column=2, sticky="w", pady=5)
        self.wfuzz_hl = tk.StringVar(value=self.config_data.get("wfuzz_hl", ""))
        self.wfuzz_hl_entry = tk.Entry(filter_frame, textvariable=self.wfuzz_hl, bg=self.bg_color, fg=self.header_color, insertbackground=self.header_color, font=("Consolas", 10), bd=0, highlightbackground=self.button_color, highlightthickness=1)
        self.wfuzz_hl_entry.grid(row=1, column=3, sticky="ew", padx=10, pady=5, ipady=3)
        self.wfuzz_hl_entry.bind("<KeyRelease>", self.update_previews)
    ...
    def setup_hydra_frame(self):
        ...
        tk.Label(frame, text="HYDRA LOGIN BRUTEFORCE AUDITOR", font=("Segoe UI", 14, "bold"), bg=self.card_color, fg=self.header_color).pack(anchor="w", pady=(0, 15))
        
        # Split Layout: Left Controls, Right Web-Form Parameters
        self.hydra_split = tk.Frame(frame, bg=self.card_color)
        self.hydra_split.pack(fill="both", expand=True)
        self.hydra_split.columnconfigure(0, weight=1, minsize=250)
        self.hydra_split.columnconfigure(1, weight=1, minsize=320)
        
        # Left Panel (Standard Controls)
        left_pane = tk.Frame(self.hydra_split, bg=self.card_color)
        left_pane.grid(row=0, column=0, sticky="nsew", padx=(0, 15))
        
        tk.Label(left_pane, text="Target Protocol / Method:", bg=self.card_color, fg=self.text_color, font=("Segoe UI", 10)).pack(anchor="w", pady=(0, 5))
        self.hydra_proto = tk.StringVar(value="ssh")
        self.hydra_proto_combo = ttk.Combobox(left_pane, textvariable=self.hydra_proto, values=["ssh", "ftp", "telnet", "http-get-form", "http-post-form"], state="readonly")
        self.hydra_proto_combo.pack(fill="x", pady=(0, 10))
        self.hydra_proto_combo.bind("<<ComboboxSelected>>", self.on_hydra_proto_changed)
        
        # Username Selection Mode
        user_mode_frame = tk.Frame(left_pane, bg=self.card_color)
        user_mode_frame.pack(fill="x", pady=5)
        self.hydra_user_is_list = tk.BooleanVar(value=False)
        ttk.Checkbutton(user_mode_frame, text="Use Username List", variable=self.hydra_user_is_list, command=self.on_hydra_user_mode_changed).pack(side="left")
        
        self.hydra_user = tk.StringVar(value=self.config_data.get("hydra_user", "admin"))
        self.hydra_user_entry = tk.Entry(left_pane, textvariable=self.hydra_user, bg=self.bg_color, fg=self.header_color, insertbackground=self.header_color, font=("Consolas", 10), bd=0, highlightbackground=self.button_color, highlightthickness=1)
        self.hydra_user_entry.pack(fill="x", pady=5, ipady=4)
        self.hydra_user_entry.bind("<KeyRelease>", self.update_previews)
        
        self.hydra_user_browse = tk.Button(left_pane, text="Browse List...", font=("Segoe UI", 9), bg=self.button_color, fg=self.header_color, activebackground=self.button_hover, activeforeground=self.header_color, bd=0, padx=10, state="disabled", command=lambda: self.browse_wordlist(self.hydra_user))
        self.hydra_user_browse.pack(anchor="e", pady=(0, 10))
        
        # Password Dictionary Wordlist
        tk.Label(left_pane, text="Password Wordlist:", bg=self.card_color, fg=self.text_color, font=("Segoe UI", 10)).pack(anchor="w", pady=(5, 5))
        
        entry_frame = tk.Frame(left_pane, bg=self.card_color)
        entry_frame.pack(fill="x")
        self.hydra_wl_var = tk.StringVar(value=self.config_data.get("hydra_wl", self.default_wl_path))
        self.hydra_wl_entry = tk.Entry(entry_frame, textvariable=self.hydra_wl_var, bg=self.bg_color, fg=self.header_color, insertbackground=self.header_color, font=("Consolas", 10), bd=0, highlightbackground=self.button_color, highlightthickness=1)
        ...
        browse_btn = tk.Button(entry_frame, text="Browse...", font=("Segoe UI", 9), bg=self.button_color, fg=self.header_color, activebackground=self.button_hover, activeforeground=self.header_color, bd=0, padx=15, command=lambda: self.browse_wordlist(self.hydra_wl_var))
        browse_btn.pack(side="right")
        
        # Additional Settings
        opts_frame = tk.Frame(left_pane, bg=self.card_color, pady=10)
        opts_frame.pack(fill="x")
        
        self.hydra_exit_first = tk.BooleanVar(value=self.config_data.get("hydra_exit_first", True))
        ttk.Checkbutton(opts_frame, text="Exit on first credential found (-f)", variable=self.hydra_exit_first, command=self.update_previews).pack(anchor="w", pady=2)
        
        speed_frame = tk.Frame(left_pane, bg=self.card_color, pady=5)
        speed_frame.pack(fill="x")
        tk.Label(speed_frame, text="Tasks Thread Count (-t):", bg=self.card_color, fg=self.text_color).pack(side="left")
        self.hydra_tasks = tk.StringVar(value="16")
        tasks_combo = ttk.Combobox(speed_frame, textvariable=self.hydra_tasks, values=["4", "8", "16", "32", "64"], width=6, state="readonly")
        tasks_combo.pack(side="left", padx=10)
        tasks_combo.bind("<<ComboboxSelected>>", self.update_previews)
        
        # Right Panel (Web Form Settings - Enabled for web forms)
        self.right_pane = tk.Frame(self.hydra_split, bg=self.card_color, bd=1, highlightbackground=self.button_color, highlightcolor=self.button_color, highlightthickness=1, padx=12, pady=12)
        
        tk.Label(self.right_pane, text="WEB FORM PARAMETERS", font=("Segoe UI", 10, "bold"), bg=self.card_color, fg=self.accent_color).pack(anchor="w", pady=(0, 10))
        
        tk.Label(self.right_pane, text="Form Login Path:", bg=self.card_color, fg=self.text_color).pack(anchor="w", pady=2)
        self.hydra_form_path = tk.StringVar(value="/login.php")
        self.hydra_form_path_entry = tk.Entry(self.right_pane, textvariable=self.hydra_form_path, bg=self.bg_color, fg=self.header_color, font=("Consolas", 9), bd=0, highlightbackground=self.button_color, highlightthickness=1)
        self.hydra_form_path_entry.pack(fill="x", pady=(0, 10), ipady=3)
        self.hydra_form_path_entry.bind("<KeyRelease>", self.update_previews)
        
        tk.Label(self.right_pane, text="Body Structure (^USER^ & ^PASS^):", bg=self.card_color, fg=self.text_color).pack(anchor="w", pady=2)
        self.hydra_form_body = tk.StringVar(value="username=^USER^&password=^PASS^&login=Submit")
        self.hydra_form_body_entry = tk.Entry(self.right_pane, textvariable=self.hydra_form_body, bg=self.bg_color, fg=self.header_color, font=("Consolas", 9), bd=0, highlightbackground=self.button_color, highlightthickness=1)
        self.hydra_form_body_entry.pack(fill="x", pady=(0, 10), ipady=3)
        self.hydra_form_body_entry.bind("<KeyRelease>", self.update_previews)
        
        tk.Label(self.right_pane, text="Failure String Match:", bg=self.card_color, fg=self.text_color).pack(anchor="w", pady=2)
        self.hydra_form_fail = tk.StringVar(value="F=Username or password incorrect")
        self.hydra_form_fail_entry = tk.Entry(self.right_pane, textvariable=self.hydra_form_fail, bg=self.bg_color, fg=self.header_color, font=("Consolas", 9), bd=0, highlightbackground=self.button_color, highlightthickness=1)
        self.hydra_form_fail_entry.pack(fill="x", pady=(0, 5), ipady=3)
    ...
    def browse_wordlist(self, target_variable):
        ...
        if selected_file:
            target_variable.set(selected_file.replace("/", "\\"))
            self.update_previews()
    ...
    def get_generated_command(self):
        ...
        if self.active_tab == 0: # Nmap
            ...
            nmap_xml_path = os.path.join(tempfile.gettempdir(), "cracken5_nmap.xml")
            return f"nmap {' '.join(flags)} -oX \"{nmap_xml_path}\" {clean_host}"
            
        elif self.active_tab == 1: # DIRB
            flags = []
            if self.dirb_silent.get(): flags.append("-S")
            
            # DIRB expects target URL to have a trailing slash for directory scanning
            dirb_url = clean_url
            if not dirb_url.endswith("/"):
                # If target has a file extension like index.php, strip it for DIRB directory scope
                if "." in dirb_url.split("/")[-1]:
                    dirb_url = "/".join(dirb_url.split("/")[:-1]) + "/"
                else:
                    dirb_url += "/"
            
            wl = self.wordlist_var.get().strip()
            if wl:
                return f"dirb {dirb_url} \"{wl}\" {' '.join(flags)}".strip()
            else:
                return f"dirb {dirb_url} {' '.join(flags)}".strip()
                
        elif self.active_tab == 2: # Wfuzz
            flags = [f"--hc {self.wfuzz_hc.get().strip()}"] if self.wfuzz_hc.get().strip() else []
            if self.wfuzz_hh.get().strip(): flags.append(f"--hch {self.wfuzz_hh.get().strip()}")
            if self.wfuzz_hw.get().strip(): flags.append(f"--hw {self.wfuzz_hw.get().strip()}")
            if self.wfuzz_hl.get().strip(): flags.append(f"--hl {self.wfuzz_hl.get().strip()}")
            
            # Wfuzz targets directories using the FUZZ placeholder key
            fuzz_url = clean_url
            if not fuzz_url.endswith("/FUZZ") and "/FUZZ" not in fuzz_url:
                if "." in fuzz_url.split("/")[-1]:
                    fuzz_url = "/".join(fuzz_url.split("/")[:-1]) + "/FUZZ"
                else:
                    fuzz_url = fuzz_url.rstrip("/") + "/FUZZ"
                    
            wl = self.wfuzz_wl_var.get().strip()
            return f"wfuzz -c -z file,\"{wl}\" {' '.join(flags)} {fuzz_url}"
            
        elif self.active_tab == 3: # Hydra
            proto = self.hydra_proto.get()
            flags = []
            
            # Username list vs static name
            if self.hydra_user_is_list.get():
                user_val = self.hydra_user.get().strip()
                flags.append(f"-L \"{user_val}\"")
            else:
                user_val = self.hydra_user.get().strip()
                flags.append(f"-l \"{user_val}\"")
                
            # Password list is always loaded
            pass_wl = self.hydra_wl_var.get().strip()
            flags.append(f"-P \"{pass_wl}\"")
            
            if self.hydra_exit_first.get():
                flags.append("-f")
                
            flags.append(f"-t {self.hydra_tasks.get()}")
            base_cmd = f"hydra {' '.join(flags)}"
    ...
    def execute_command(self):
        ...
        # Generate temporary batch script for Windows Command Prompt execution
        script_content = f"@echo off\n{cmd}\necho.\necho Scan complete.\npause\n"
        
        try:
            fd, temp_path = tempfile.mkstemp(suffix=".bat", prefix="cracken5_")
        ...
        try:
            # Launch Windows Command Prompt detached running the batch script
            subprocess.Popen(["cmd.exe", "/c", "start", "Cr4cKen5 Attack Window", temp_path], shell=True)
        except Exception as e:
            messagebox.showerror("Execution Failed", f"Could not launch Command Prompt:\n{str(e)}")

    def extract_default_wordlist(self):
        import sys
        try:
            # PyInstaller temp extraction path
            base_path = sys._MEIPASS
        except Exception:
            base_path = os.path.abspath(".")
            
        src_path = os.path.join(base_path, "default_wordlist.txt")
        
        # Copy to local Temp folder for CLI tools to read
        if os.path.exists(src_path):
            try:
                shutil.copy2(src_path, self.default_wl_path)
            except Exception:
                pass
        else:
            # Emergency fallback content if file doesn't exist
            try:
                with open(self.default_wl_path, "w") as f:
                    f.write("admin\npassword\n123456\nroot\nguest\nadministrator\n12345\nqwerty\npass\nuser\n")
            except Exception:
                pass
    ...
    def load_findings(self):
        ...
        # 1. Parse Nmap XML Output (Windows Temp)
        nmap_xml_path = os.path.join(tempfile.gettempdir(), "cracken5_nmap.xml")
        ...
        # 2. Parse SQLMap Logs (Windows Paths check)
        sqlmap_logs = []
        # Check standard user folder locations
        sqlmap_logs.extend(glob.glob(os.path.expanduser("~/.sqlmap/output/*/log")))
        sqlmap_logs.extend(glob.glob(os.path.expanduser("~/.local/share/sqlmap/output/*/log")))
...
"""
        ...
        report_path = os.path.join(tempfile.gettempdir(), "cracken5_security_report.html")
        try:
